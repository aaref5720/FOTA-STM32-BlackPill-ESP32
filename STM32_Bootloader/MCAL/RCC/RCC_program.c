...
static void RCC_void_PLL_Init(void)
{
    ...
    SET_BIT(RCC_CR,RCC_CR_HSEON_Pos);
    while (GET_BIT(RCC_CR,RCC_CR_HSERDY_Pos) == 0);
    /* Enable PLL and wait until it is locked */
    SET_BIT(RCC_CR,RCC_CR_PLLON_Pos);
    while (GET_BIT(RCC_CR,RCC_CR_PLLRDY_Pos) == 0);
    /* Set flash wait states before switching to higher frequency */
    FPEC_ACR = 0x02;
